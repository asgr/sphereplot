rgl.globe = function(type = 'world', radius=1, res=100, alpha=2/3, grid=TRUE, lit=FALSE, ...){
  
  type = tolower(type)
  
  if(type=='world' | type=='earth'){
    longtype = 'D'
    if(radius=='auto'){
      radius = 6371
    }
    texture = system.file("extdata", 'world.png', package="sphereplot")
